...
#import "WSYLaunchAdManager.h"
#import <XHLaunchAd/XHLaunchAd.h>
#import "WSYLaunchAdModel.h"
#import "WSYWebViewController.h"
...
@interface WSYLaunchAdManager()<XHLaunchAdDelegate>

@end

@implementation WSYLaunchAdManager

+(void)load{
    [self shareManager];
}

+(WSYLaunchAdManager *)shareManager{
    static WSYLaunchAdManager *instance = nil;
    static dispatch_once_t oneToken;
    dispatch_once(&oneToken,^{
        instance = [[WSYLaunchAdManager alloc] init];
    });
    return instance;
}
...
-(void)example01{
    ...
    //广告数据请求
    [WSYNetworking getLaunchAdImageWithParameters:nil success:^(id response) {
        NSLog(@"广告数据 = %@",response);
        //广告数据转模型
        WSYLaunchAdModel *model = [WSYLaunchAdModel mj_objectWithKeyValues:response[@"data"]];
        //配置广告数据
...
-(void)xhLaunchAd:(XHLaunchAd *)launchAd clickAndOpenModel:(id)openModel clickPoint:(CGPoint)clickPoint{
    ...
    WSYWebViewController *VC = [[WSYWebViewController alloc] init];
