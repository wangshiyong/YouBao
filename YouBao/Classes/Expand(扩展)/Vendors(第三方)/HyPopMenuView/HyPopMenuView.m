...
#define CancelStrImgaeName @"M_takePhone"
...
- (void)openMenu
{
    ...
    UILabel *lab = [[UILabel alloc]initWithFrame:CGRectMake(0, NAV_HEIGHT, kW, 50)];
    _headLab = lab;
    _headLab.text = @"记录我的旅行";
    _headLab.font = [UIFont systemFontOfSize:35];
    _headLab.textAlignment = NSTextAlignmentCenter;
    _headLab.textColor = [UIColor blackColor];
    [_backgroundView addSubview:_headLab];
...
- (void)closeMenu
{
    ...
    [UIView animateKeyframesWithDuration:Duration delay:d options:0 animations:^{
        if (isAlpha) {
            ...
            UIVisualEffectView *effect = (id)self.backgroundView;
            effect.effect = nil;
...
- (CGRect)getFrameAtIndex:(NSUInteger)index;
{
    ...
    CGFloat buttonViewY = ((rowIndex + 1) * margin + rowIndex * buttonViewHeight) + (kH - toHeight) - kH/2;
...
- (__kindof UIView*)effectsViewWithType:(HyPopMenuViewBackgroundType)type
{
    ...
    if (gradientLayer) {
        ...
        effectBlur = [UIBlurEffect effectWithStyle:UIBlurEffectStyleExtraLight];
        UIVisualEffectView *visualEffectView =[[UIVisualEffectView alloc]initWithEffect:effectBlur];
        visualEffectView.frame = effectView.frame;
